  ...
  const handleOAuthSignIn = async (provider: OAuthProvider) => {
    ...
    try {
      ...
      const { error: oauthError } = await supabase.auth.signInWithOAuth({
        provider,
        options: {
          redirectTo: `${window.location.origin}/auth/callback?next=/auth/check-mfa-redirect`,
        },
      })
    ...
    <div className="min-h-screen flex items-center justify-center bg-white p-4">
      <Card className="w-full max-w-md shadow-lg border-2 border-gray-200">
        <CardHeader className="space-y-3 text-center pb-6">
          ...
          <div className="inline-flex items-center gap-2 px-4 py-2 bg-[#FFE5B4] border border-[#D97706] rounded-full mx-auto mt-2">
            <Construction className="h-4 w-4 text-[#D97706]" />
            <span className="text-[#D97706] text-xs font-semibold uppercase tracking-wide">
        ...
        <CardContent className="space-y-6">
          ...
          <div className="space-y-3 text-center">
            <p className="text-sm text-gray-600">
              ...
              <Link
                href="/auth/signup"
                className="font-medium text-[#D97706] hover:text-[#B45309] hover:underline transition-colors"
              >
                Create one now
              </Link>
            </p>
